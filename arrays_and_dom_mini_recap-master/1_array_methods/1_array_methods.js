...
const friendS = data.filter((person) => {
...
const howLiveNy = data.filter((person) => {
  if (person.address.state === 'NY') {
    return person.address.state
  }
});
console.log(howLiveNy[0].name);


// Exercise 3: Use the filter method to get all the people who are older than 30
const peopleOld = data.filter((person) => {
  if (person.age > 30) {
    return true;
  }
  return false;
});
console.log(peopleOld[0].name + " " + peopleOld[0].age);
